...
import { cartDeleteThunk, cartFindThunk } from "../../services/cart-thunks";
...
const Cart = () => {
    ...
    return (<>
        ...
        <div className="pb-3 h5 border-bottom d-flex flex-row justify-content-between">
            <div>Loan Cart</div>
            {cartFull && <Button danger onClick={() => dispatch(cartDeleteThunk(profile.username))}>Delete Cart</Button>}
        </div>
        {!cartFull && <div className="mb-5"> <b>No books added to the cart</b></div>}
        {cartFull && cart.books && allBooks && cart.books
            .map((cartBook, idx) => {
                debugger
